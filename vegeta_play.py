...
from vegeta import (
    load_frame, render_sprite, POSES, QUOTES, HERO_QUOTES,
)
...
class Sprite:
    def __init__(self, frames, scale=1):
        # frames: list of PIL RGBA images
        # Pre-render each frame as list-of-(x, ch, attr_key)
        self.tiles = [self._tilize(f, scale) for f in frames]
        ref = frames[0].resize((frames[0].width*scale, frames[0].height*scale), Image.NEAREST)
        self.w = ref.width
    ...
    def _tilize(self, img, scale):
        ...
        if h % 2:
            ...
            h += 1
        px = img.load()
        tiles = []  # (row, col, char, fg_rgb_or_None, bg_rgb_or_None)
        for y in range(0, h, 2):
            row = y // 2
            for x in range(w):
                tp = px[x, y]; bp = px[x, y+1]
                tt = tp[3] < 64; bt = bp[3] < 64
                if tt and bt: continue
                if tt:
...
class ColorPool:
    """Curses dynamic color allocator. Maps (r,g,b) fg + optional (r,g,b) bg → color pair."""
    def __init__(self):
        self.colors = {}   # (r,g,b) → color idx (stops after MAX)
        self.pairs = {}    # (fg_idx, bg_idx) → pair idx
        self.color_next = 16
        self.pair_next = 1
        self.max_colors = min(curses.COLORS, 256) if hasattr(curses, "COLORS") else 256
        self.max_pairs = min(curses.COLOR_PAIRS, 256) if hasattr(curses, "COLOR_PAIRS") else 256
        self.default_bg = -1
        try:
    ...
    def _color_idx(self, rgb):
        if rgb is None: return self.default_bg
        if rgb in self.colors: return self.colors[rgb]
        if self.color_next >= self.max_colors:
            # approximate with nearest already-allocated color
            best, bd = self.default_bg, 10**9
            for c, idx in self.colors.items():
                d = (c[0]-rgb[0])**2 + (c[1]-rgb[1])**2 + (c[2]-rgb[2])**2
                if d < bd: bd, best = d, idx
            return best
        idx = self.color_next; self.color_next += 1
        try:
            curses.init_color(idx,
                int(rgb[0]*1000/255), int(rgb[1]*1000/255), int(rgb[2]*1000/255))
        except curses.error:
    ...
    def pair(self, fg_rgb, bg_rgb):
        ...
        key = (fi, bi)
        if key in self.pairs: return curses.color_pair(self.pairs[key])
        if self.pair_next >= self.max_pairs:
            return curses.A_NORMAL
        idx = self.pair_next; self.pair_next += 1
        try:
...
def draw_sprite(win, sprite, frame_idx, row, col, flip, pool, maxy, maxx):
    ...
    w = sprite.w
    for r, c, ch, fg, bg in tiles:
        x = col + (w - 1 - c if flip else c)
        y = row + r
        if 0 <= y < maxy and 0 <= x < maxx:
            try:
                win.addstr(y, x, ch, pool.pair(fg, bg))
            except curses.error:
                pass
...
def play():
    def _main(stdscr):
        ...
        try:
            ...
            print("\x1b[?1003h", end="", flush=True)  # enable any-event mouse tracking
        except Exception:
            pass

        pool = ColorPool()

        # Pre-load sprites at chosen scale
        scale = 2
        idle = Sprite([load_frame(i) for i in [10, 11]], scale=scale)
        walk = Sprite([load_frame(i) for i in [43, 44, 45, 46]], scale=scale)
        powerup = Sprite([load_frame(i) for i in POSES["powerup"]], scale=scale)

        maxy, maxx = stdscr.getmaxyx()
        y = maxy - idle.h - 1
        x = maxx // 2 - idle.w // 2
        vx = 1  # auto-walk direction
        flip = False
        state = "walk"
        state_frame = 0
        state_timer = 0
        anim_timer = 0.0  # time-based frame advance for idle/walk
        auto_walk = True
        quote = None
        quote_until = 0
        quote_x = x
        ...
        while True:
            ...
            while ch != -1:
                if ch in (ord('q'), 27):
                    ...
                    x -= 2; flip = True; auto_walk = False
                    if state != "walk": state = "walk"; state_frame = 0
                elif ch == curses.KEY_RIGHT or ch == ord('l'):
                    x += 2; flip = False; auto_walk = False
                    if state != "walk": state = "walk"; state_frame = 0
                elif ch == curses.KEY_UP or ch == ord('k'):
                    y -= 1; auto_walk = False
                    if state == "powerup": state = "walk"; state_frame = 0
                elif ch == curses.KEY_DOWN or ch == ord('j'):
                    y += 1; auto_walk = False
                    if state == "powerup": state = "walk"; state_frame = 0
                elif ch == ord(' '):
                    state = "powerup"; state_frame = 0; state_timer = now
                    say(random.choice(HERO_QUOTES), dur=3.0)
                elif ch == ord('t'):
                    say(random.choice(QUOTES), dur=3.5)
                elif ch == ord('a'):
                    auto_walk = True; state = "walk"
            ...
            # State machine
            if state == "powerup":
                # Advance powerup frames, then return to walk
                ...
                # idle/walk auto-animate at a steady ~8 FPS regardless of loop rate
                if now - anim_timer > 0.125:
                    state_frame += 1
                    anim_timer = now

            # Current sprite for bounds
            cur_sp = powerup if state == "powerup" else (walk if (auto_walk or state == "walk") else idle)
            ...
            if auto_walk and state != "powerup":
                ...
                if x < 0:
                    x = 0; vx = 1; flip = False
                if x + cur_sp.w > maxx:
                    x = maxx - cur_sp.w; vx = -1; flip = True
            ...
            # Random taunt
            if now >= next_auto_quote and (quote is None or now > quote_until):
                pool_q = QUOTES
                say(random.choice(pool_q), dur=3.5)
