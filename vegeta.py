#!/usr/bin/env python3
"""Vegeta — your terminal motivator.
Usage:
  vegeta                  greet (random pose + quote)
  vegeta greet [pose]     greet, optionally forcing a pose group
  vegeta powerup [pose]   full power-up, optionally forcing a pose group
  vegeta play             interactive mode (arrows to move, q to quit)
  vegeta list             show all available frames
  Pose groups: stance_a, stance_b, charge, crouch, step, dash, super,
               recover, punch, beam, land, pose
"""
import os, sys, random, time, shutil, glob, argparse
...
def render_sprite(img, scale=1, bg_color=None):
    ...
    scale: integer upscale factor (nearest-neighbor).
    bg_color: terminal bg color (r,g,b) for transparent pixels; None = use ANSI default bg.
    ...
    for y in range(0, h, 2):
        ...
        for x in range(w):
            tp = px[x, y]
            bp = px[x, y+1]
            # Transparency → use term default bg
            top_t = tp[3] < 64
            bot_t = bp[3] < 64
            if top_t and bot_t:
                # Both transparent: emit a plain space (reset colors)
                line.append(RESET + " ")
                last_top = last_bot = None
                continue
            if top_t:
                # Transparent top, colored bottom: use ▄ with bg reset
                line.append(f"{RESET}{fg(bp[0],bp[1],bp[2])}▄")
                last_top = last_bot = None
                continue
            if bot_t:
                # Colored top, transparent bottom: use ▀
                line.append(f"{RESET}{fg(tp[0],tp[1],tp[2])}▀")
                last_top = last_bot = None
                continue
            # Both opaque: ▀ with fg=top, bg=bottom
            line.append(f"{fg(tp[0],tp[1],tp[2])}{bg(bp[0],bp[1],bp[2])}▀")
            last_top, last_bot = tp, bp
        line.append(RESET)
...
def _render_centered_sequence(frames, target_h):
    """Render pose frames on a fixed-size canvas, each anchored at center-x
    and feet-to-bottom. Keeps Vegeta rooted in one spot across frames of
    varying widths/heights (no more jitter)."""
...
def cmd_greet(args):
    ...
    if group == "beam":
        ...
        print()
        return
    # Only use frames that actually exist on disk (some indices were filtered
    # out during slicing). Preserves order → animation stays coherent.
    pool = [i for i in POSES[group]
...
def _play_animation(renders, h, delay=0.18):
    """Play an in-place animation, clearing each line before redraw (no ghosting)."""
...
def _play_galick_gun(target_h=20, delay=0.09):
    ...
    beam that extends rightward to the edge of the terminal. Source sprites
    already face RIGHT natively, so no horizontal flip is applied."""
    charge_imgs = [load_frame(i) for i in (86, 89, 90, 91, 92, 93)]
    fire_img = load_frame(94)
    burst = load_frame(96)
    head = load_frame(97)

    # Canvas width is clamped to the terminal to prevent line wrapping, which
    # breaks the cursor-up math in _play_animation.
    term_cols = shutil.get_terminal_size().columns
    canvas_w = max(40, term_cols - 6)

    # Pose scale: fit pose to target_h rows, but cap so pose takes at most
    # half the canvas width (leaves room for the beam to traverse).
    ...
    def up(im, s=ps):
        return im.resize((max(1, im.width * s), max(1, im.height * s)), Image.NEAREST)
    ...
    # Beam body = middle vertical strip of the head, tiled horizontally.
    mid_x = head_s.width // 2
    strip_w = max(2, ps * 2)
    body_strip = head_s.crop((mid_x - strip_w // 2, 0, mid_x + strip_w // 2, head_s.height))
    ...
    # Phase 1 — charge: cycle the cupped-hand frames twice.
    for _ in range(2):
        for im in charge_s:
            frames.append(composite(im, []))

    # Phase 2 — firing pose held visibly before anything leaves his hands.
    for _ in range(3):
        frames.append(composite(fire_s, []))

    # Phase 3 — burst blooms at the hands the moment the beam ignites.
    burst_x = hand_x - burst_s.width // 3
    burst_y = hand_y - burst_s.height // 2
    frames.append(composite(fire_s, [(burst_s, burst_x, burst_y)]))
    frames.append(composite(fire_s, [(burst_s, burst_x, burst_y)]))

    # Phase 4 — continuous beam: body grows from hand to the far edge of the
    # canvas, head leads the tip. Step size derived from canvas width so the
    # beam reaches the edge regardless of terminal size.
    start_x = hand_x
    ...
    # Hold final full-length beam for a beat.
...
def cmd_list(args):
    ...
    # print every frame index with size
    for f in files:
        im = Image.open(f)
...
def main():
    p = argparse.ArgumentParser(prog="vegeta")
    sub = p.add_subparsers(dest="cmd")

    g = sub.add_parser("greet");   g.add_argument("pose", nargs="?", default=None)
    pu = sub.add_parser("powerup"); pu.add_argument("pose", nargs="?", default=None)
    sub.add_parser("list")
    s = sub.add_parser("show"); s.add_argument("index", type=int)
    sub.add_parser("play")

    args = p.parse_args()
    cmd = args.cmd or "greet"
    if cmd == "greet":   cmd_greet(args)
    elif cmd == "powerup": cmd_powerup(args)
    elif cmd == "list":  cmd_list(args)
    elif cmd == "show":  cmd_show(args)
    elif cmd == "play":
        from vegeta_play import play
        play()
